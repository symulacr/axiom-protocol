...
contract FuzzAxiomTeeVerifierTest is StdInvariant, Test {
    ...
    function setUp() public {
        ...
        _forkViable = true;
        try vm.load(address(0x0000000000000000000000000000000000000465), bytes32(0)) {}
        catch {
            _forkViable = false;
        }

        // Only configure invariant targets when the fork is archive-viable.
        if (_forkViable) {
            targetContract(address(verifier));
            bytes4[] memory selectors = new bytes4[](1);
            selectors[0] = AxiomTeeVerifier.proposeSigner.selector;
    ...
    function testFuzz_verifyTransferValidity_validProof_succeeds(
        ...
        TransferValidityProofOutput[] memory outs = verifier.verifyTransferValidity(proofs, randomReceiver, address(this));
    ...
    function testFuzz_verifyTransferValidity_wrongSigner_reverts(
        ...
        bytes32 ownershipMsg = _ownershipDigest(dataHash, sealedKey, pub, address(0), address(this), nonce, validUntil);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(attackerKeySeed, ownershipMsg);
        bytes memory ownershipSig = abi.encodePacked(r, s, v);

        bytes32 accessMsg = _accessDigest(dataHash, pub, address(0), address(this), nonce, validUntil);
        ...
        verifier.verifyTransferValidity(proofs, address(0), address(this));
    }

    /// @notice Fuzz wrong access message: OwnershipProof signed correctly,
    ///         AccessProof signed over mutated data.
    function testFuzz_verifyTransferValidity_wrongAccessMessage_reverts(
        uint8 seed
    ) public {
        ...
        (uint8 v, bytes32 r, bytes32 s) =
            vm.sign(TEE_KEY, _ownershipDigest(dataHash, sealedKey, pub, address(0), address(this), nonce, validUntil));
        bytes memory ownershipSig = abi.encodePacked(r, s, v);
        bytes32 tamperedDataHash = bytes32(uint256(dataHash) ^ 0xDEADBEEF);
        bytes32 wrongAccessMsg = _accessDigest(tamperedDataHash, pub, address(0), address(this), nonce, validUntil);
        ...
        try verifier.verifyTransferValidity(proofs, address(0), address(this)) returns (
            TransferValidityProofOutput[] memory
        ) {
    ...
    function testFuzz_verifyTransferValidity_truncatedSignature_reverts(
        uint8 seed
    ) public {
    ...
    function testFuzz_verifyTransferValidity_zeroLengthSignature_reverts(
        uint8 seed
    ) public {
    ...
    function testFuzz_verifyTransferValidity_inBatchReplay_reverts(
        uint8 seed
    ) public {
        uint256 receiverKeySeed = bound(uint256(keccak256(abi.encodePacked(seed, "rx4"))), 1, SECP256K1_ORDER_MINUS_1);
        address receiver = vm.addr(receiverKeySeed);
        ...
        verifier.verifyTransferValidity(dup, receiver, address(this));
    }
    ...
    function testFuzz_verifyTransferValidity_emptyBatch_succeeds() public {
        ...
        TransferValidityProofOutput[] memory outs = verifier.verifyTransferValidity(empty, address(0), address(this));
    ...
    function _batchHelper(
        uint256 n,
        uint8 salt
    ) internal {
        // A single batch verifies against one recipient (`to`).
        uint256 rk = bound(uint256(keccak256(abi.encodePacked(salt, "rx-batch"))), 1, SECP256K1_ORDER_MINUS_1);
        address receiver = vm.addr(rk);
        ...
        TransferValidityProofOutput[] memory outs = verifier.verifyTransferValidity(batch, receiver, address(this));
    ...
    function testFuzz_verifyTransferValidity_validUntilPast_reverts(
        ...
        uint256 receiverKeySeed =
            bound(uint256(keccak256(abi.encodePacked(seed, "rx-past"))), 1, SECP256K1_ORDER_MINUS_1);
        address randomReceiver = vm.addr(receiverKeySeed);
    ...
    function testFuzz_verifyTransferValidity_validUntilAtNow_succeeds(
        uint8 seed
    ) public {
        uint256 receiverKeySeed =
            bound(uint256(keccak256(abi.encodePacked(seed, "rx-now"))), 1, SECP256K1_ORDER_MINUS_1);
        address randomReceiver = vm.addr(receiverKeySeed);
        ...
        bytes32 ownershipMsg = _ownershipDigest(dataHash, sealedKey, pub, randomReceiver, address(this), nonce, validUntil);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(TEE_KEY, ownershipMsg);
        bytes memory ownershipSig = abi.encodePacked(r, s, v);

        bytes32 accessMsg = _accessDigest(dataHash, pub, randomReceiver, address(this), nonce, validUntil);
        (v, r, s) = vm.sign(receiverKeySeed, accessMsg);
        bytes memory accessSig = abi.encodePacked(r, s, v);
    ...
    function testFuzz_verifyTransferValidity_validUntilFuture_succeeds(
        ...
        uint256 receiverKeySeed =
            bound(uint256(keccak256(abi.encodePacked(seed, "rx-future"))), 1, SECP256K1_ORDER_MINUS_1);
        address randomReceiver = vm.addr(receiverKeySeed);
    ...
    function testFuzz_verifyTransferValidity_validUntilTooFar_reverts(
        ...
        uint256 receiverKeySeed =
            bound(uint256(keccak256(abi.encodePacked(seed, "rx-toofar"))), 1, SECP256K1_ORDER_MINUS_1);
        address randomReceiver = vm.addr(receiverKeySeed);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AxiomTeeVerifier.AxiomValidUntilTooFar.selector, validUntil, block.timestamp, uint256(7 days)
            )
        );
        verifier.verifyTransferValidity(proofs, address(0), address(this));
    }
    ...
    function test_verifyTransferValidity_validUntilOverflow_reverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AxiomTeeVerifier.AxiomValidUntilTooFar.selector, type(uint256).max, block.timestamp, uint256(7 days)
            )
        );
        verifier.verifyTransferValidity(proofs, address(0), address(this));
    }
    ...
    function testFuzz_verifyTransferValidity_warpPast_validUntilReverts(
        uint8 seed
    ) public {
        uint256 receiverKeySeed =
            bound(uint256(keccak256(abi.encodePacked(seed, "rx-warp"))), 1, SECP256K1_ORDER_MINUS_1);
    ...
    function test_liveForkBytecode_containsMaxProofAgeSelector() public {
        // Skip if env var not available.
        try vm.envAddress("AXIOM_TEE_VERIFIER_ADDRESS") returns (address) {}
        catch {
            vm.skip(true);
            return;
        }
        // The live Wave E-5 verifier ships the fix — the selector IS present.
        address live = vm.envAddress("AXIOM_TEE_VERIFIER_ADDRESS");
        ...
        assertTrue(
            found, "live AxiomTeeVerifier must contain the maxProofAgeSeconds() selector (Wave E-5 fix deployed)"
        );
    }
    ...
    /// @notice Fuzz the zero-address guard. Owner cannot propose zero address.
    function testFuzz_proposeSigner_zeroAddress_reverts(
        uint8 seed
    ) public {
        vm.prank(owner);
    ...
    function testFuzz_proposeSigner_strangerReverts(
        uint256 strangerKeySeed,
        uint256 newSignerKeySeed
    ) public {
        if (!_forkViable) vm.skip(true);
        return;
        ...
        vm.expectRevert(abi.encodeWithSelector(OwnableUpgradeable.OwnableUnauthorizedAccount.selector, someStranger));
        verifier.proposeSigner(someNewSigner);
    ...
    function testFuzz_cleanExpiredProofs_keepsLiveExpiresExpired(
        ...
        for (uint256 i = 0; i < numProofsSeed; i++) {
            uint256 receiverKeySeed =
                bound(uint256(keccak256(abi.encodePacked("clean-mixed", seed, i))), 1, SECP256K1_ORDER_MINUS_1);
            address receiver = vm.addr(receiverKeySeed);
        ...
        for (uint256 i = 0; i < numProofsSeed; i++) {
            ...
            if (i >= numProofsSeed / 2) {
                uint256 receiverKeySeed =
                    bound(uint256(keccak256(abi.encodePacked("clean-live", seed, i))), 1, SECP256K1_ORDER_MINUS_1);
                address receiver = vm.addr(receiverKeySeed);
                ...
                verifier.verifyTransferValidity(live, receiver, address(this));
        ...
        for (uint256 i = 0; i < deadIdx; i++) {
            uint256 receiverKeySeed =
                bound(uint256(keccak256(abi.encodePacked("clean-resubmit", seed, i))), 1, SECP256K1_ORDER_MINUS_1);
            address randomReceiver = vm.addr(receiverKeySeed);
            ...
            (uint8 v, bytes32 r, bytes32 s) = vm.sign(
                TEE_KEY, _ownershipDigest(dataHash, sealedKey, pub, randomReceiver, address(this), nonce, validUntil)
            );
            bytes memory ownershipSig = abi.encodePacked(r, s, v);
            (v, r, s) =
                vm.sign(receiverKeySeed, _accessDigest(dataHash, pub, randomReceiver, address(this), nonce, validUntil));
            bytes memory accessSig = abi.encodePacked(r, s, v);
            ...
            TransferValidityProofOutput[] memory outs =
                verifier.verifyTransferValidity(fresh, randomReceiver, address(this));
    ...
    ///         signers and `proposeSigner` rejects zero signers, so this
    ///         must always hold.
    function test_invariant_registeredSignerNeverZero() public {
        if (!_forkViable) vm.skip(true);
        return;
    ...
    function test_invariant_maxProofAgeConstant() public {
        if (!_forkViable) vm.skip(true);
        return;
    ...
    function _makeValidProof(
        ...
        return _makeValidProofWithValidUntil(
            teeKey, receiverKey, dataHash, nonce, sealedKey, to, nft, block.timestamp + DEFAULT_VALID_UNTIL_OFFSET
        );
    }
