...
contract FuzzAxiomTeeVerifierTest is StdInvariant, Test {
    ...
    function testFuzz_verifyTransferValidity_validProof_succeeds(
        ...
        TransferValidityProofOutput[] memory outs = verifier.verifyTransferValidity(proofs, randomReceiver, address(0));
    ...
    function testFuzz_verifyTransferValidity_wrongAccessMessage_reverts(
        ...
        (uint8 v, bytes32 r, bytes32 s) =
            vm.sign(TEE_KEY, _ownershipDigest(dataHash, sealedKey, pub, address(0), address(0), nonce, validUntil));
        bytes memory ownershipSig = abi.encodePacked(r, s, v);
        ...
        try verifier.verifyTransferValidity(proofs, address(0), address(0)) returns (
    ...
    function testFuzz_verifyTransferValidity_inBatchReplay_reverts(
        ...
        verifier.verifyTransferValidity(dup, receiver, address(0));
    }
    ...
    function _batchHelper(
        ...
        TransferValidityProofOutput[] memory outs = verifier.verifyTransferValidity(batch, receiver, address(0));
    ...
    function testFuzz_verifyTransferValidity_validUntilAtNow_succeeds(
        ...
        bytes32 ownershipMsg = _ownershipDigest(dataHash, sealedKey, pub, randomReceiver, address(0), nonce, validUntil);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(TEE_KEY, ownershipMsg);
        bytes memory ownershipSig = abi.encodePacked(r, s, v);

        bytes32 accessMsg = _accessDigest(dataHash, pub, randomReceiver, address(0), nonce, validUntil);
        (v, r, s) = vm.sign(receiverKeySeed, accessMsg);
        bytes memory accessSig = abi.encodePacked(r, s, v);
    ...
    function testFuzz_verifyTransferValidity_warpPast_validUntilReverts(
        ...
        verifier.verifyTransferValidity(proofs, receiver, address(0));
    }
    ...
    function testFuzz_cleanExpiredProofs_keepsLiveExpiresExpired(
        ...
        for (uint256 i = 0; i < numProofsSeed; i++) {
            ...
            if (i >= numProofsSeed / 2) {
                ...
                verifier.verifyTransferValidity(live, receiver, address(0));
        ...
        for (uint256 i = 0; i < deadIdx; i++) {
            ...
            (uint8 v, bytes32 r, bytes32 s) = vm.sign(
                TEE_KEY, _ownershipDigest(dataHash, sealedKey, pub, randomReceiver, address(0), nonce, validUntil)
            );
            bytes memory ownershipSig = abi.encodePacked(r, s, v);
            (v, r, s) =
                vm.sign(receiverKeySeed, _accessDigest(dataHash, pub, randomReceiver, address(0), nonce, validUntil));
            bytes memory accessSig = abi.encodePacked(r, s, v);
