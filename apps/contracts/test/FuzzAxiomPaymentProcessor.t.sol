...
contract FuzzAxiomPaymentProcessorUnit is Test {
    ...
    /// @notice Migration is blocked while outstanding creator earnings exist.
    function testFuzz_setPaymentToken_revertsWithOutstandingEarnings(
        uint256 amount
    ) public {
    ...
    ///         token as `oldToken`.
    function testFuzz_setPaymentToken_emitsEvent(
        address newToken
    ) public {
        vm.assume(newToken != address(0));
    ...
    function testFuzz_payForAgent_happySplits(
    ...
    function testFuzz_payForAgent_revertsWithoutApproval(
        uint256 amount
    ) public {
        amount = bound(amount, 1, type(uint128).max);
        token.mint(payer, amount);
    ...
    function testFuzz_payForAgent_revertsOnZeroAmount(
        uint256 agentTokenId
    ) public {
        vm.prank(payer);
    ...
    function testFuzz_payForAgent_revertsOnNonStandardToken(
        uint256 amount
    ) public {
        amount = bound(amount, 1, type(uint128).max);
    ...
    function testFuzz_withdrawAgentEarnings_sweeps(
        uint128 targetEarnings
    ) public {
        ...
        assertGe(token.balanceOf(address(processor)), actualEarnings, "processor holds creator's cut");
    ...
    function testFuzz_withdrawAgentEarnings_reentrancyBlocked(
        uint128
    ) public {
    ...
    function testFuzz_payComputeProvider_happy(
        address provider,
        uint256 amount
    ) public {
    ...
    function testFuzz_payComputeProvider_revertsOnZeroProvider(
        uint256 amount
    ) public {
        amount = bound(amount, 1, type(uint128).max);
        token.mint(payer, amount);
    ...
    function testFuzz_payComputeProvider_revertsOnZeroAmount(
        address provider
    ) public {
        vm.assume(provider != address(0));
        vm.prank(payer);
    ...
    // ─── V3 (W1-A): cap + ratio bound fuzz ─────────────────────────────

    /// @notice The compute lane inherits the MAX_PAY cap through _payTransferFrom:
    ///         payComputeProvider reverts PayAmountExceedsCap iff amount > cap.
    function testFuzz_payComputeProvider_capEnforced(
        uint256 cap,
        uint256 amount
    ) public {
        cap = bound(cap, 1, type(uint128).max);
        ...
        if (amount > cap) {
            vm.prank(payer);
            vm.expectRevert(abi.encodeWithSelector(AxiomPaymentProcessor.PayAmountExceedsCap.selector, amount, cap));
            processor.payComputeProvider(address(0xD3F), amount);
        } else {
            vm.prank(payer);
            processor.payComputeProvider(address(0xD3F), amount);
            assertEq(token.balanceOf(address(0xD3F)), amount, "at/below-cap compute pay forwarded");
        }
    }

    /// @notice The compute leg of payForAgentAndCompute reverts when computeAmount exceeds
    ///         computeRatioMax * agentAmount; boundary (equal) is allowed. Cap is disabled
    ///         here so the ratio bound is the only tripping check.
    function testFuzz_payForAgentAndCompute_ratioBoundEnforced(
        uint256 ratioMax,
        uint256 agentAmount,
        uint256 computeAmount
    ) public {
        ratioMax = bound(ratioMax, 1, type(uint32).max);
        agentAmount = bound(agentAmount, 1, type(uint64).max);
        computeAmount = bound(computeAmount, 1, type(uint128).max);
        vm.prank(admin);
        processor.setComputeRatioMax(ratioMax);

        uint256 maxCompute = ratioMax * agentAmount;
        // The agent leg must also pass the split-side checks: creator registered, and
        // maxCompute must not overflow uint256 when compared (bounded inputs guarantee this).
        token.mint(payer, agentAmount + computeAmount);
        vm.prank(payer);
        token.approve(address(processor), agentAmount + computeAmount);

        if (computeAmount > maxCompute) {
            vm.prank(payer);
            vm.expectRevert(
                abi.encodeWithSelector(AxiomPaymentProcessor.ComputeRatioExceeded.selector, computeAmount, maxCompute)
            );
            processor.payForAgentAndCompute(AGENT_TOKEN_ID, address(0xD3F), agentAmount, computeAmount);
        } else {
            vm.prank(payer);
            processor.payForAgentAndCompute(AGENT_TOKEN_ID, address(0xD3F), agentAmount, computeAmount);
            assertEq(token.balanceOf(address(0xD3F)), computeAmount, "boundary-allowed compute leg paid");
        }
    }

    /// @notice setComputeRatioMax is an ADMIN_ROLE op; anyone else reverts and storage is untouched.
    function testFuzz_setComputeRatioMax_revertsForNonAdmin(
        address caller,
        uint256 ratioMax
    ) public {
        vm.assume(caller != owner);
        ...
        processor.setComputeRatioMax(ratioMax);
        assertEq(processor.computeRatioMax(), 0, "ratio bound unchanged on revert");
    }
    ...
    function test_liveAddress_hasNoCode_atPinnedBlock() public {
        ...
        try vm.envAddress("AXIOM_PAYMENT_PROCESSOR_ADDRESS") returns (address) {}
        catch {
            vm.skip(true);
            return;
        }
        ...
        address live = vm.envAddress("AXIOM_PAYMENT_PROCESSOR_ADDRESS");
        // EXTCODESIZE is the cheapest "has code" check.
        uint256 size;
        assembly {
            size := extcodesize(live)
        }
        assertEq(size, 0, "payment processor had no code at pinned block (deployed after)");
    }

    /// @notice Same probe at the latest block. The new Galileo deploy
    ///         (2026-06-25) should have code at the latest block.
    function test_liveAddress_hasCode_atLatestBlock() public {
        // Skip if env var not available.
        try vm.envAddress("AXIOM_PAYMENT_PROCESSOR_ADDRESS") returns (address) {}
        catch {
            vm.skip(true);
        ...
        assertGt(size, 0, "live AxiomPaymentProcessor should have code at latest block");
    }
}
...
contract ProcessorHandler is Test {
    ...
    constructor(
        AxiomPaymentProcessor _processor,
        MockERC20 _token,
        address _creator
    ) {
    ...
    function payForAgentFuzz(
        ...
        try processor.payForAgent(tokenId, amount) {
            ...
            ghostTotalCreatorEarnings += (processor.agentEarningsOf(creator) - creatorEarningsBefore);
    ...
    function payComputeProviderFuzz(
        uint256 payerSeed,
        uint256 amount
    ) external {
        address p = _pickPayer(payerSeed);
        amount = bound(amount, 1, 1_000_000 ether);
        address provider = address(uint160(uint256(keccak256(abi.encode(p, amount, ghostCallCount)))));
...
contract FuzzAxiomPaymentProcessorInvariants is StdInvariant, Test {
    ...
    function invariant_authorizedTokenTransfersBalanced() public view {
        ...
        assertEq(processorBalance, creatorEarnings, "processor token balance must equal creator earnings slot");
    }
}
