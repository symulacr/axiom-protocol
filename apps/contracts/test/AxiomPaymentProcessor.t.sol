...
contract AxiomPaymentProcessorTest is Test {
    ...
    function test_proposeProtocolTreasury_emitsEvent() public {
        ...
        uint256 expectedEffectiveAt = block.timestamp + 10 minutes;
    ...
    // ─── V3 (W1-A): capped compute lanes ─────────────────────────────
    function test_payComputeProvider_revertsWhenOverCap() public {
        uint256 cap = 500e6;
        vm.prank(owner);
        processor.setMaxPayCap(cap);

        token.mint(payer, cap + 1);
        vm.prank(payer);
        token.approve(address(processor), cap + 1);

        vm.expectRevert(abi.encodeWithSelector(AxiomPaymentProcessor.PayAmountExceedsCap.selector, cap + 1, cap));
        vm.prank(payer);
        processor.payComputeProvider(address(0xD3F), cap + 1);
    }

    function test_payComputeProvider_atCapBoundary() public {
        uint256 cap = 500e6;
        vm.prank(owner);
        processor.setMaxPayCap(cap);

        token.mint(payer, cap);
        vm.prank(payer);
        token.approve(address(processor), cap);

        vm.expectEmit(true, false, false, true);
        emit AxiomPaymentProcessor.ComputeProviderPaid(address(0xD3F), cap);
        vm.prank(payer);
        processor.payComputeProvider(address(0xD3F), cap);

        assertEq(token.balanceOf(address(0xD3F)), cap, "provider paid full at-cap amount");
    }

    function test_payForAgentAndCompute_computeLegRevertsWhenOverCap() public {
        uint256 cap = 1000e6;
        vm.prank(owner);
        processor.setMaxPayCap(cap);

        // Ratio bound is unset (0 = unlimited), so the cap alone must trip the revert.
        token.mint(payer, 200e6 + cap + 1);
        vm.prank(payer);
        token.approve(address(processor), 200e6 + cap + 1);

        vm.expectRevert(abi.encodeWithSelector(AxiomPaymentProcessor.PayAmountExceedsCap.selector, cap + 1, cap));
        vm.prank(payer);
        processor.payForAgentAndCompute(AGENT_TOKEN_ID, address(0xD3F), 200e6, cap + 1);

        assertEq(processor.agentEarningsOf(creator), 0, "no earnings persisted on reverted tx");
    }

    function test_payForAgentAndCompute_bothLegsAtCapBoundary() public {
        uint256 cap = 1000e6;
        vm.prank(owner);
        processor.setMaxPayCap(cap);

        uint256 agentAmount = cap;
        uint256 computeAmount = cap;
        token.mint(payer, agentAmount + computeAmount);
        vm.prank(payer);
        token.approve(address(processor), agentAmount + computeAmount);
        ...
        uint256 expectedProtocolCut = (agentAmount * PROTOCOL_FEE_BPS) / 10_000;
        assertEq(token.balanceOf(address(0xD3F)), computeAmount, "provider paid at-cap compute leg");
        assertEq(processor.agentEarningsOf(creator), agentAmount - expectedProtocolCut, "creator credited at-cap agent leg");
    }

    // ─── V3 (W1-A): agentAmount→computeAmount ratio bound ────────────
    function test_defaultComputeRatioMaxIsZero_unlimited() public {
        assertEq(processor.computeRatioMax(), 0, "default 0 = unlimited, matches maxPayCap semantics");
    }

    function test_setComputeRatioMax_adminSetsAndEmits() public {
        vm.expectEmit(false, false, false, true);
        emit AxiomPaymentProcessor.ComputeRatioMaxUpdated(0, 10);
        vm.prank(owner);
        processor.setComputeRatioMax(10);
        assertEq(processor.computeRatioMax(), 10, "ratio bound stored");
    }

    function test_setComputeRatioMax_revertsForNonAdmin() public {
        bytes32 adminRole = processor.ADMIN_ROLE();
        vm.prank(payer);
        vm.expectRevert(abi.encodeWithSelector(IAccessControl.AccessControlUnauthorizedAccount.selector, payer, adminRole));
        processor.setComputeRatioMax(5);
    }

    function test_payForAgentAndCompute_revertsWhenRatioExceeded() public {
        vm.prank(owner);
        processor.setComputeRatioMax(2);

        uint256 agentAmount = 100e6;
        uint256 computeAmount = 201e6; // > 2 * agentAmount
        token.mint(payer, agentAmount + computeAmount);
        vm.prank(payer);
        token.approve(address(processor), agentAmount + computeAmount);

        vm.expectRevert(
            abi.encodeWithSelector(AxiomPaymentProcessor.ComputeRatioExceeded.selector, computeAmount, 2 * agentAmount)
        );
        vm.prank(payer);
        processor.payForAgentAndCompute(AGENT_TOKEN_ID, address(0xD3F), agentAmount, computeAmount);

        assertEq(processor.agentEarningsOf(creator), 0, "creator split unchanged on reverted tx");
    }

    function test_payForAgentAndCompute_ratioBoundaryAllowed() public {
        vm.prank(owner);
        processor.setComputeRatioMax(2);

        uint256 agentAmount = 100e6;
        uint256 computeAmount = 200e6; // exactly 2 * agentAmount
        token.mint(payer, agentAmount + computeAmount);
        vm.prank(payer);
        token.approve(address(processor), agentAmount + computeAmount);
        ...
        uint256 expectedProtocolCut = (agentAmount * PROTOCOL_FEE_BPS) / 10_000;
        assertEq(token.balanceOf(address(0xD3F)), computeAmount, "boundary compute leg paid in full");
        assertEq(processor.agentEarningsOf(creator), agentAmount - expectedProtocolCut, "boundary agent leg split normally");
    }

    function test_setComputeRatioMax_zeroDisablesRatioBound() public {
        vm.prank(owner);
        processor.setComputeRatioMax(2);
        vm.prank(owner);
        processor.setComputeRatioMax(0);

        uint256 agentAmount = 1;
        uint256 computeAmount = 1000e6;
        token.mint(payer, agentAmount + computeAmount);
        vm.prank(payer);
        token.approve(address(processor), agentAmount + computeAmount);
        ...
        vm.prank(payer);
        processor.payForAgentAndCompute(AGENT_TOKEN_ID, address(0xD3F), agentAmount, computeAmount);
        assertEq(token.balanceOf(address(0xD3F)), computeAmount, "uncapped ratio after bound cleared");
    }
}
