...
contract AxiomMetadataJsonTest is Test {
    ...
    function test_decisionDocumented_sentinelEventEmitted() public {
        ...
        emit MetadataJsonDecisionDocumented("Axiom Agent NFT", "AXM-A", "2RH-REJECTED-v1");
    ...
    function test_metadataJsonOf_escapesSpecialChars() public {
        ...
        IntelligentData[] memory datas = new IntelligentData[](1);
        datas[0] = IntelligentData({dataDescription: "evil\"quote\\\\backslash", dataHash: keccak256("escape-test")});
        uint256 tokenId = nft.mint(datas, alice);

        string memory json = nft.metadataJsonOf(tokenId);
        // Verify escaping sequences are present.
        assertTrue(_contains(json, string(bytes(hex"5c22"))), "quote is escaped");
    ...
    function _base64Decode(
        ...
        for (uint256 i = 0; i < 64; i++) {
            table[uint8(alpha[i])] = bytes1(SafeCast.toUint8(i));
        }
        ...
        for (uint256 i = 0; i < inb.length; i += 4) {
            ...
            // Mask after shifting: n is uint32, so (n >> 8) and (n) include high bits without mask.
            // & 0xFF extracts exactly the target byte position.
            if (oi < outLen) out[oi++] = bytes1(SafeCast.toUint8((n >> 16) & 0xFF));
            if (oi < outLen) out[oi++] = bytes1(SafeCast.toUint8((n >> 8) & 0xFF));
            if (oi < outLen) out[oi++] = bytes1(SafeCast.toUint8(n & 0xFF));
        }
        return out;
    }
}
