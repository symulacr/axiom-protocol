...
contract FuzzAxiomAgentNFT is StdInvariant, LiveForkTest {
    // ─── Live proxy + verifier on Galileo (read from env vars) ─────
    AxiomAgentNFT internal liveNft;
    AxiomTeeVerifier internal liveVerifier;
    ...
    function setUp() public {
        ...
        try vm.envUint("AXIOM_OPERATOR_PK") returns (uint256 pk) {
            OPERATOR_PK = pk;
        } catch {
            vm.skip(true);
            return;
        ...
        // Read addresses from env vars (never hardcoded in source).
        liveNft = AxiomAgentNFT(vm.envAddress("AGENT_NFT_ADDRESS"));
        liveVerifier = AxiomTeeVerifier(payable(vm.envAddress("AXIOM_TEE_VERIFIER_ADDRESS")));
        operator = vm.envAddress("AXIOM_OPERATOR_ADDRESS");
        receiver1 = vm.envAddress("AXIOM_TEST_RECEIVER_1_ADDRESS");
        receiver2 = vm.envAddress("AXIOM_TEST_RECEIVER_2_ADDRESS");
        ...
        assertEq(liveNft.name(), "Axiom Agent NFT", "live proxy name mismatch");
        assertEq(liveNft.symbol(), "AXM-A", "live proxy symbol mismatch");
        assertTrue(
            vm.load(address(liveNft), EIP1967_IMPL_SLOT) != bytes32(0),
        ...
        // Bind labels for readable traces.
        vm.label(operator, "OPERATOR");
        vm.label(receiver1, "RECEIVER_1");
        vm.label(receiver2, "RECEIVER_2");
        vm.label(address(liveNft), "LIVE_NFT_PROXY");
        vm.label(address(liveVerifier), "LIVE_VERIFIER");
    ...
    function testFuzz_mintWithRole_recordsAllFields(
        ...
        vm.assume(receiver != address(liveNft));
        vm.assume(receiver != address(liveVerifier));
        ...
        uint256 actualTokenId = liveNft.mintWithRole(data, receiver, creator);
        vm.stopPrank();

        // (a) TokenId increments monotonically
        assertEq(actualTokenId, expectedTokenId, "tokenId must equal nextTokenId pre-mint");
        assertGe(actualTokenId, liveNextTokenIdBefore, "tokenId must be >= pre-fork high-water mark");

        // (b) creatorOf mapping is set correctly. W1-C: a zero-creator arg now defaults
        // to the receiver (matching the 2-arg mintWithRole intent) instead of leaving
        // the token permanently reverted in the Processor's _paySplit.
        if (creator != address(0)) {
            assertEq(liveNft.creatorOf(actualTokenId), creator, "creatorOf mismatch");
        } else {
            assertEq(liveNft.creatorOf(actualTokenId), receiver, "creatorOf must default to receiver when creator arg is 0");
        }

        // (c) The dataHash stored matches the input.
        IntelligentData[] memory stored = liveNft.intelligentDatasOf(actualTokenId);
    ...
    function testFuzz_authorizeUsage_accessControl(
        ...
        // Mint a fresh token to receiver1 so the test is self-contained on the live proxy.
        ...
        vm.prank(operator);
        uint256 tokenId = liveNft.mintWithRole(data, receiver1, receiver1);
        _recordDataHash(tokenId, freshHash);
        _bumpHighWater(tokenId + 1);

        // Case 1: caller == owner (receiver1) — should succeed and append userToAuthorize.
        address[] memory before = liveNft.authorizedUsersOf(tokenId);
        uint256 beforeLen = before.length;

        vm.prank(receiver1);
        liveNft.authorizeUsage(tokenId, userToAuthorize);

        address[] memory afterList = liveNft.authorizedUsersOf(tokenId);
        assertEq(afterList.length, beforeLen + 1, "authorizedUsersOf must grow by 1 after owner-authorizes");
        ...
        if (caller != receiver1 && caller != address(0)) {
            vm.prank(caller);
            // The contract reverts with ERC721IncorrectOwner when caller != owner.
            vm.expectRevert();
            liveNft.authorizeUsage(tokenId, address(uint160(uint256(keccak256(abi.encodePacked("deny", caller))))));
        }

        // Case 3: zero-address user is rejected (the contract checks this first).
        vm.prank(receiver1);
        vm.expectRevert();
        liveNft.authorizeUsage(tokenId, address(0));
    }
    ...
    function testFuzz_iTransferFrom_doesNotClearData(
        ...
        vm.assume(receiver != address(0));
        vm.assume(receiver != receiver1);
        vm.assume(fuzzedDataHash != bytes32(0));

        // Mint a fresh token to receiver1.
        IntelligentData[] memory data = new IntelligentData[](1);
        ...
        proofs[0] = TransferValidityProof({
            accessProof: AccessProof({
                dataHash: fuzzedDataHash, targetPubkey: pub, nonce: abi.encode(nonce), proof: accessSig, validUntil: validUntil
            }),
        ...
        bytes32 preDataHash = liveNft.intelligentDatasOf(tokenId)[0].dataHash;

        if (receiver == receiver1) {
            // Happy path: real signer signed the access proof.
            vm.prank(receiver1);
            liveNft.iTransferFrom(receiver1, receiver1, tokenId, proofs);
            // (d) dataHash is NOT cleared.
            assertEq(
                liveNft.intelligentDatasOf(tokenId)[0].dataHash,
                preDataHash,
                "dataHash must not be cleared by iTransferFrom"
            );
        } else {
            // Negative path: receiver is fuzzed, but receiver1 signed.
            vm.prank(receiver1);
            vm.expectRevert();
            liveNft.iTransferFrom(receiver1, receiver, tokenId, proofs);
        }
    ...
    function invariant_totalSupplyMonotonic() public view {
        ...
        assertGe(current, highWaterNextTokenId, "nextTokenId regressed below the high-water mark");
    }
    ...
    function _readNextTokenId() internal view returns (uint256) {
        return uint256(vm.load(address(liveNft), CLONEABLE_STORAGE_SLOT));
    }

    function _recordDataHash(
        uint256,
        /* tokenId */
        bytes32 h
    ) internal {
...
contract FuzzAxiomAgentNFTSanity is LiveForkTest {
    bytes32 internal constant EIP1967_IMPL_SLOT = 0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc;
    ...
    function test_sanity_proxyLive() public {
        _skipUnlessLiveFork();
        address nftAddr;
        try vm.envAddress("AGENT_NFT_ADDRESS") returns (address addr) {
            nftAddr = addr;
        } catch {
            vm.skip(true);
            return;
        ...
        // Non-archive RPCs fail here — skip instead of failing CI.
        try this._probeLiveProxy(nftAddr) returns (bool ok) {
            if (!ok) {
                vm.skip(true);
                return;
            }
        ...
        assertEq(liveNft.name(), "Axiom Agent NFT");
        assertEq(liveNft.symbol(), "AXM-A");
        assertTrue(vm.load(address(liveNft), EIP1967_IMPL_SLOT) != bytes32(0));
        uint256 nextId = uint256(vm.load(address(liveNft), CLONEABLE_STORAGE_SLOT));
...
contract FuzzAxiomAgentNFTLocal is Test {
    ...
    bytes constant ALICE_PUB64 =
        hex"8517ac9f78ea4ac7d1b49080b2b4dfae7f9a74706196ff07054a2487ec6aeef4ac91c131cdafda05d68788a64269d079bec396a26901732c45eca768402f27c7";
    bytes constant BOB_PUB64 =
        hex"1d3015ac205ab30d45136c50fd02acceb9ee36a564ba4bbab360503994c4d00d0aff28a0f89ce9534b2f4f55000ea0b540f783c75ea46d2d84b6934e021fbe99";
    bytes constant CAROL_PUB64 =
        hex"e2c27802172b6a2f02217de87f502211850f257f1fe5f66cf05928467f6aeae788ee4a5bc7ee7c817b224be835df5bd04b85affd4c7121b801689ccd83e493e2";
    bytes constant ADMIN_PUB64 =
        hex"704bad530a71af03b909fe53754132a6ed93eefb530beda9ea0a2eb70e0bfcf6734138928c2a65d1252ef13a005710a9e8b2d7a4e584a47d8646e6b66b439ed1";
    ...
    function testFuzz_iCloneFrom_succeeds(
        bytes32 dataHash,
        uint256 nonce
    ) public {
        vm.assume(dataHash != bytes32(0));
    ...
    function testFuzz_iCloneFrom_preservesOriginal(
        bytes32 dataHash,
        uint256 nonce
    ) public {
        vm.assume(dataHash != bytes32(0));
    ...
    function testFuzz_iCloneFrom_unauthorizedCallerReverts(
        bytes32 dataHash,
        uint256 nonce,
        address caller
    ) public {
