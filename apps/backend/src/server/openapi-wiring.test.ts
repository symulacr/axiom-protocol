 ...
 *
 * Proves that `apps/backend/docs/openapi.json` (generated by `generate:openapi` from
 * REGISTERED_ROUTES + zod schemas) matches how the frontend and chat-runtime actually
 * call the backend. Pure static contract check: reads the spec JSON, resolves $refs,
 * and asserts per-consumer existence / params / bodies / responses. No server boot,
 * no network, no new deps (bun:test + node:fs).
 *
 ...
 *
 * KNOWN SPEC DEFECT (2026-08-14, generator in apps/backend/scripts — Agent A): every
 * requestBody content schema is emitted as raw zod-4 `.def` (`{def:{type:"object",
 * shape:{…}}}`) instead of a proper SchemaObject / `$ref` to the registered component
 * (components.schemas.TickBody / ChatBody / MintEncodeBody / … exist and are proper).
 * This is the documented zod-to-openapi v9 `registerComponent` gotcha (deep plan §(b)1):
 * use `registry.register(name, schema)` so request bodies transform properly. Until the
 * generator is fixed, the "request bodies" test below fails with per-consumer
 * "declared: none" — that failure IS the contract working; it goes green after regen.
 * -----------------------------------------------------------------------------------
 */
