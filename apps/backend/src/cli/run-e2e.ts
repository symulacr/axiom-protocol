...
async function main(): Promise<void> {
  ...
  const health = (await (await fetch(`${BACKEND_URL}/health`)).json()) as {
    ok: boolean;
    signer: string;
    chainHead: number;
  };
  ...
  const deployerPub = Buffer.concat([
    new Uint8Array([0x04]),
    deriveUncompressedPubkeyFromHex(DEPLOYER_PK),
  ]);
  ...
  const accessProofSignature = receiver.signingKey.sign(
  ...
  const finalResp = await postStep<FinalResponse>(
    ...
    (r) => ({
      ...
      ok:
        r.ok === true &&
        r.stage === "final" &&
        r.accessSigner?.toLowerCase() === to.toLowerCase(),
    }),
  );
  console.log(
    `          AccessProof signer ${finalResp.accessSigner} matches receiver ${to}`,
  );
  ...
  if (currentOwner.toLowerCase() !== deployer.address.toLowerCase()) {
    console.log(
      `          Skip: tokenId=${tokenId} already owned by ${currentOwner} (not deployer).`,
    );
    stepResults.push({
      step: 10,
    ...
    try {
      ...
      await nftTc.raw
        ...
        .staticCall(deployer.address, to, BigInt(tokenId), proofs);
      ...
      if (eventFrom.toLowerCase() !== deployer.address.toLowerCase())
      ...
      const recoveredAddr = recoverAccessSigner(
        finalResp.accessProof.proof,
        accessInput,
        eip712Domain,
      );
      if (recoveredAddr.toLowerCase() !== to.toLowerCase())
