...
const CARDS: ScenarioBreakCard[] = [
  {
    ...
    canBreak: [
      "storage node sync timeout",
      "insufficient OG for storage fee",
      "RPC unreachable",
    ],
    layer: "storage",
    detect: "Step 4 throws before rootHash",
    improveBackend:
      "Retry with backoff; surface storage fee estimate in /health",
  ...
  },
  {
    ...
    canBreak: [
      "insufficient OG for mintFee",
      "dataHash not registered",
      "RPC receipt flake",
    ],
    layer: "contract",
  ...
  },
  {
    ...
    canBreak: [
      "legacy 3-arg vs 4-arg ABI skew",
      "invalid Merkle root",
      "not vault owner",
    ],
    layer: "contract",
    ...
    improveBackend:
      "vault-compat already handles; expose variant in /v1/payment/config",
  ...
  },
  {
    ...
    canBreak: [
      "compute 401",
      "model not found",
      "split-brain event store (2 backends)",
    ],
    layer: "compute",
  ...
  },
  {
    ...
    improveBackend:
      "Apply limit to eventName queries; default sort newest-first",
  },
  {
  ...
  },
  {
    ...
    improveBackend:
      "CDX timeout + cached snapshots; separate slow job for /snapshots",
  },
];
...
export function printScenarioBreakReport(): void {
  ...
  console.log(
    "  • Operator wallet: single nonce lane — pipeline send, batch-wait receipts",
  );
  console.log(
    "  • Disjoint lanes: HTTP/WS ∥ on-chain reads; post-mint HTTP ∥ vault deposit+strategy",
  );
  console.log(
    "  • Mega lane (default): view-sweep reads ∥ withdraw+authorize+update+royalty pipeline",
  );
  console.log(
    "  • ~2× wall time: E2E_REUSE_TOKEN=1 (after E2E_KEEP_TOKEN=1 seed) or E2E_SKIP_VAULT_WITHDRAW=1",
  );
}
