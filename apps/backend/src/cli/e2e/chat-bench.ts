...
async function runKeepAliveBench(deps: {
  backendUrl: string;
  computeModel: string;
  rounds: number;
  liveCompute: boolean;
...
async function runContextGrowthBench(deps: {
  backendUrl: string;
  computeModel: string;
  rounds: number;
  liveCompute: boolean;
...
async function runModelSwitchBench(deps: {
  backendUrl: string;
  models: string[];
  liveCompute: boolean;
}): Promise<ChatBenchResult> {
...
async function runLiveChatToolsBench(deps: {
  backendUrl: string;
  computeModel: string;
  liveCompute: boolean;
}): Promise<ChatBenchResult> {
...
async function resolveBenchModels(backendUrl: string): Promise<string[]> {
...
export async function runChatBench(deps: {
  ...
  const chainId = deps.chainId ?? ARISTOTLE_CHAIN_ID;
