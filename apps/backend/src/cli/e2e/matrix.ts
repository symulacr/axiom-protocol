...
export function computeParityReport(): ParityReport {
...
export function assertParityGate(minActionablePct: number): void {
  const report = computeParityReport();
  if (report.actionablePct < minActionablePct) {
    throw new Error(
      `Parity gate failed: ${report.actionablePct}% < ${minActionablePct}% (${report.actionableCovered}/${report.actionable} actionable functions)`,
    );
  }
}