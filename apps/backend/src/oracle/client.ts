...
interface TransferValidityInput {
...
interface TransferValidityResult extends OwnershipProofResultWithMeta {
	validUntil?: string;
}
...
export class DefaultSignerOracleClient implements OracleClient {
	...
	private async request<T>(
		...
		const res = await fetch(`${this.baseUrl}${path}`, {
			...
			headers,
			body:
				body !== undefined ? JSON.stringify(body, bigintReplacer) : undefined,
