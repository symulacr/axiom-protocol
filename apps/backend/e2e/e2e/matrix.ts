...
export function initParityMatrix(): void {
  ...
  for (const [fn, reason] of [
    ...
    ["proposeVerifier", "ADMIN_ROLE + 1-day timelock"],
    ["executeVerifier", "ADMIN_ROLE + timelock elapsed"],
    ["cancelVerifierProposal", "ADMIN_ROLE admin path"],
    ["setMintFee", "ADMIN_ROLE"],
    ["setStorageInfo", "ADMIN_ROLE"],
    // V2 (ADR-004 §1.1): withdrawal is timelocked (proposeFeeWithdrawal → 1 day → executeFeeWithdrawal).
    [
      "withdrawMintFees",
      "superseded by timelocked proposeFeeWithdrawal/executeFeeWithdrawal (ADMIN_ROLE)",
    ],
    ["proposeFeeWithdrawal", "ADMIN_ROLE + 1-day timelock"],
    ["executeFeeWithdrawal", "ADMIN_ROLE + timelock elapsed"],
    ["cancelFeeWithdrawal", "ADMIN_ROLE admin path"],
    ["pendingFeeWithdrawal", "view (V2 timelock)"],
    ["pendingFeeWithdrawalExecutableAt", "view (V2 timelock)"],
    ["proposeUpgrade", "ADMIN_ROLE + 1-day timelock"],
    ["executeUpgrade", "ADMIN_ROLE + timelock elapsed"],
    ["cancelUpgrade", "ADMIN_ROLE admin path"],
    ["pendingUpgrade", "view (V2 timelock)"],
    ["pendingUpgradeExecutableAt", "view (V2 timelock)"],
    ["pause", "ADMIN_ROLE"],
    ["unpause", "ADMIN_ROLE"],
