...
// V3 W4 statefold (ex-AxiomStateView): one-call pre-flight reads from the
// PaymentProcessor. 503 ADDRESS_NOT_CONFIGURED until the processor address
// is configured; ?payer= defaults to the zero address (aggregate view).
const agentStateVaultHealthRef = reg(
  "AgentStateVaultHealth",
  z.object({
    balance: bigintStr,
    strategyRoot: hexStr.meta({
      description:
        "bytes32 Merkle root of allowed (target,selector) pairs; zero = no strategy",
    }),
    dailyLimit: bigintStr,
    dailySpent: bigintStr,
    resetDay: bigintStr.meta({
      description: "UTC day index the window resets on",
    }),
    validUntilDay: bigintStr.meta({ description: "0 = no expiry" }),
    expired: z.boolean(),
  }),
);
const agentStatePaymentSnapshotRef = reg(
  "AgentStatePaymentSnapshot",
  z.object({
    maxPayCap: bigintStr.meta({
      description: "Payment cap (single-primitive, V3)",
    }),
    computeRatioMax: bigintStr,
    agentBalance: bigintStr.meta({
      description: "Vault balance for the agent",
    }),
    payerAllowance: bigintStr.meta({
      description: "Payer token allowance to the processor",
    }),
    paymentToken: addressStr.meta({
      description: "ERC-20 payment token address",
    }),
  }),
);
const agentStateResponseRef = reg(
  "AgentStateResponse",
  z.object({
    tokenId: tokenIdStr,
    payer: agentStatePaymentSnapshotRef.optional(),
    vaultHealth: agentStateVaultHealthRef.optional(),
    errors: z
      .record(z.string(), z.string())
      .optional()
      .meta({
        description:
          "Per-read failure detail when one view fails but the other succeeded",
      }),
  }),
);

// V3 W5-B relayer surface + V3 W6-B faucet (all 503 ADDRESS_NOT_CONFIGURED
// while the GasTank address is unset).
const relayerTankRef = reg(
  "RelayerTank",
  z.object({
    address: addressStr,
    balance: bigintStr.meta({ description: "Tank balance in wei" }),
    grants: bigintStr.meta({ description: "Grants consumed" }),
    grantsCap: bigintStr,
    grantsLeft: bigintStr,
    gasGrant: bigintStr.meta({
      description: "Live per-grant size in wei (admin-tunable, never hardcoded)",
    }),
    opsLeft: z
      .number()
      .int()
      .meta({ description: "Approx ops remaining at gasGrant granularity" }),
    reserve: bigintStr.meta({ description: "Protocol gas reserve in wei" }),
  }),
);
const relayerStatusRef = reg(
  "RelayerStatus",
  z.object({
    mode: z.enum(["on", "off"]),
    address: addressStr.nullable(),
    relayer: addressStr.nullable(),
    sponsorMaxGasCostWei: bigintStr,
    sponsorRatePerMin: z.number().int(),
    sponsorMaxInflightPerUser: z.number().int(),
    batchMax: z.number().int(),
    queue: z.record(z.string(), z.number().int()),
  }),
);
// Sponsor body matches the zod schema in route-schemas.ts (pattern-only
// fields — the committed spec deliberately omits the shared type metadata).
const sponsorBodyRef = reg(
  "SponsorBody",
  z.object({
    user: z.string().regex(new RegExp(ADDRESS_PATTERN)),
    target: z.string().regex(new RegExp(ADDRESS_PATTERN)),
    data: z.string().regex(new RegExp(HEX_PATTERN)),
    maxGasCost: z.string().regex(new RegExp(TOKEN_ID_PATTERN)),
    nonce: z.string().regex(new RegExp(TOKEN_ID_PATTERN)),
    deadline: z.string().regex(new RegExp(TOKEN_ID_PATTERN)),
    signature: z.string().regex(new RegExp(HEX_PATTERN)),
  }),
);
const sponsorAcceptedRef = reg(
  "SponsorAccepted",
  z.object({
    ok: z.literal(true),
    id: z.string().meta({ description: "Relayer-side correlation id" }),
    nonce: z
      .string()
      .regex(new RegExp(TOKEN_ID_PATTERN))
      .meta({
        description: "Per-user ForwardRequest nonce (echoed decimal string)",
      }),
    sponsored: z.literal(true),
  }),
);
const faucetStatusRef = reg(
  "FaucetStatus",
  z.object({
    eligible: z.boolean(),
    amount: bigintStr.meta({
      description: "Drip size in axmUSDC base units (6 decimals)",
    }),
    token: z.enum(["axmUSDC"]),
  }),
);
const faucetClaimRef = reg(
  "FaucetClaim",
  z.object({
    ok: z.literal(true),
    dripped: z.boolean(),
...
// V3 W6-B: public Pyth spot prices for the swap UI's slippage check.
const pricesResponseRef = reg(
  "PricesResponse",
  z.object({
    prices: z.array(
      z.object({
        symbol: z.string(),
        price: z.string().meta({ description: "Human price with expo applied" }),
        confidence: z
          .string()
          .meta({ description: "Pyth confidence interval" }),
        expo: z.number().int(),
        publishedAt: z
          .number()
          .int()
          .meta({ description: "Publish time (unix ms)" }),
      }),
    ),
  }),
);
...
const ROUTES = [
  ...
  // V3 W4 statefold: one-call pre-flight (paymentSnapshot + vaultHealthOf).
  {
    method: "get",
    path: "/v1/agents/{id}/state",
    summary:
      "Pre-flight agent state from the PaymentProcessor (statefold): paymentSnapshot + vaultHealthOf in one call (10s cache)",
    security: SEC.serverOrClient,
    parameters: [
      {
        name: "id",
      ...
      },
      {
        ...
        required: false,
        description:
          "Payer address to scope paymentSnapshot; defaults to the zero address (aggregate view)",
        schema: { type: "string", pattern: ADDRESS_PATTERN },
      },
    ],
    responses: {
      "200": okResp(
        "Combined snapshot; reads that failed are omitted and detailed in errors",
        agentStateResponseRef,
      ),
      "400": errorResp("Missing/invalid id"),
      "502": errorResp("Both stateview reads failed"),
      "503": errorResp("PaymentProcessor address not configured (pre-deploy)"),
    },
  },

  // V3 W5-B relayer: mode + queue stats; self-describes while off.
  {
    method: "get",
    path: "/v1/relayer/status",
    summary: "Relayer mode + queue stats",
    security: SEC.serverOrClient,
    responses: {
      "200": okResp("Relayer status", relayerStatusRef),
      "503": errorResp("Relayer not enabled", ["ADDRESS_NOT_CONFIGURED"]),
    },
  },
  {
    method: "get",
    path: "/v1/relayer/tank/{address}",
    summary: "GasTank balance/grants view for an address (read route)",
    security: SEC.serverOrClient,
    parameters: [
      {
        name: "address",
    ...
    responses: {
      "200": okResp("Tank status", relayerTankRef),
      "400": errorResp("Invalid address"),
      "502": errorResp("On-chain tank read failed", ["UPSTREAM_ERROR"]),
      "503": errorResp("GasTank address not configured", [
        "ADDRESS_NOT_CONFIGURED",
      ]),
    },
  },
  {
    method: "post",
    path: "/v1/relayer/sponsor",
    summary: "Submit an EIP-712 ForwardRequest for sponsored relay (gas-free op)",
    security: SEC.serverOrClient,
    request: {
      body: {
        description:
          "EIP-712 ForwardRequest (domain AxiomGasTank/1) + userSig; maxGasCost/nonce/deadline as decimal strings",
        required: true,
        content: json(sponsorBodyRef),
      },
    },
    responses: {
      "202": okResp("Queued for sponsored relay", sponsorAcceptedRef),
      "400": errorResp("Invalid signature / deadline passed / simulation failed", [
        "INVALID_SIGNATURE",
        "INVALID_SIGNER",
        "DEADLINE_PASSED",
        "SIMULATION_FAILED",
      ...
      "402": errorResp(
        "Tank exhausted (user-side remedy: wait/top-up/refill)",
        ["TANK_EXHAUSTED"],
      ),
      "429": errorResp("Sponsor rate limit or inflight cap", [
        "SPONSOR_RATE_LIMITED",
        "SPONSOR_INFLIGHT_LIMIT",
      ]),
      "503": errorResp("Relayer disabled or reserve depleted", [
        "ADDRESS_NOT_CONFIGURED",
        "RESERVE_DEPLETED",
      ]),
    },
  ...
  // V3 W6-B faucet: drip eligibility read + one-time drip claim.
  {
    method: "get",
    path: "/v1/relayer/faucet/{address}",
    summary: "Testnet axmUSDC faucet eligibility for an address (read route)",
    security: SEC.serverOrClient,
    parameters: [
      {
        name: "address",
    ...
    responses: {
      "200": okResp("Faucet eligibility", faucetStatusRef),
      "400": errorResp("Invalid address"),
      "503": errorResp("Relayer disabled", ["ADDRESS_NOT_CONFIGURED"]),
    },
  },
  {
    method: "post",
    ...
    responses: {
      "200": okResp(
        "Claim result (dripped=false when already fauceted or ineligible)",
        faucetClaimRef,
      ),
      "400": errorResp("Invalid address"),
      "503": errorResp("Relayer disabled", ["ADDRESS_NOT_CONFIGURED"]),
  ...
  // V3 W6-B: public market data — Pyth spot prices for the swap UI.
  {
    method: "get",
    path: "/v1/prices",
    summary: "Pyth latest prices for the top-15 feed map (30s cache)",
    security: SEC.serverOrClient,
    responses: {
      "200": okResp("Latest prices", pricesResponseRef),
      "503": errorResp("Hermes unreachable", ["PRICES_UNAVAILABLE"]),
    },
  },
