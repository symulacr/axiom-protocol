...
test("inmemory_seen_hashes_persist_across_restart", () => {
  ...
  const before = new InMemoryStorage({ seenHashesFile: file });
  assert.equal(
  ...
  const afterRestart = new InMemoryStorage({ seenHashesFile: file });
  assert.equal(
...
test("zerogstorage_seen_hashes_persist_across_restart", () => {
  ...
  const before = new ZeroGStorage(config, { seenHashesFile: file });
  assert.equal(
    before.hasSeenDataHash(dataHash),
    false,
    "fresh registry should not have seen the hash yet",
  ...
  const afterRestart = new ZeroGStorage(config, { seenHashesFile: file });
  assert.equal(
    afterRestart.hasSeenDataHash(dataHash),
    true,
    "marked hash must survive a restart (durable backing)",
