// Build both service binaries in parallel (skips tsc — bun compiles TS
...
const jobs = [
  ...
  ["apps/oracle/src/index.ts", "axiom-oracle"],
].map(([entry, name]) =>
  Bun.spawn(
...
console.log(`built 2 binaries in ${((performance.now() - t0) / 1000).toFixed(2)}s`);
