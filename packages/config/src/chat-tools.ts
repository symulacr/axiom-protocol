...
const SKILL_TOOL_DEFS = [
  ...
  skill({ name: "stocks_quote", label: "Stocks Quote", hint: "Get real-time stock price quotes", friction: "low", parameters: { type: "object", properties: { symbol: { type: "string", description: "Ticker symbol" } }, required: ["symbol"] } }),
  skill({ name: "stocks_search", label: "Stocks Search", hint: "Search for stock tickers and company names", friction: "low", parameters: { type: "object", properties: { query: { type: "string", description: "Search query" } }, required: ["query"] } }),
  skill({ name: "stocks_history", label: "Stocks History", hint: "Fetch historical OHLCV price data for equities", friction: "low", parameters: { type: "object", properties: { symbol: { type: "string", description: "Ticker symbol" }, range: { type: "string", description: "1d,5d,1mo,3mo,6mo,1y,5y,max (default 1y)" }, interval: { type: "string", description: "1m,5m,15m,1d,1wk,1mo (default 1d)" } }, required: ["symbol"] } }),
  skill({ name: "stocks_compare", label: "Stocks Compare", hint: "Compare fundamentals and performance across tickers", friction: "medium", parameters: { type: "object", properties: { symbols: { type: "array", description: "List of ticker symbols (1-10)" } }, required: ["symbols"] } }),
  ...
  skill({ name: "osint_ofac_sdn", label: "OFAC SDN", hint: "Check entities against OFAC sanctions (SDN) list", friction: "low", parameters: { type: "object", properties: { name: { type: "string", description: "Entity name" } }, required: ["name"] }, capabilities: ["osint", "ofac"], context: "external OSINT APIs" }),
  skill({ name: "osint_opencorporates", label: "OpenCorporates", hint: "Look up corporate registration and officer data", friction: "low", parameters: { type: "object", properties: { jurisdiction: { type: "string", description: "Jurisdiction code (default us)" }, query: { type: "string", description: "Company query" } }, required: ["query"] }, capabilities: ["osint", "opencorporates"], context: "external OSINT APIs" }),
  ...
  skill({ name: "osint_courtlistener", label: "CourtListener", hint: "Search US federal and state court opinions and filings", friction: "medium", parameters: { type: "object", properties: { query: { type: "string", description: "Search query" }, type: { type: "string", description: "o=opinions, r=recap (default o)" }, limit: { type: "number", description: "1-20 (default 10)" } }, required: ["query"] }, capabilities: ["osint", "courtlistener"], context: "external OSINT APIs" }),
...
export const CHAT_TOOL_CATALOG = [
  tool({ name: "list_my_agents", class: "read", label: "Your Agents", hint: "List all agent NFTs owned by the connected wallet address", requiresWallet: true, friction: "low", parameters: { type: "object", properties: {} } }),
  tool({ name: "vault_balance", class: "read", label: "Vault Balance", hint: "Get vault balance (in wei) for a given agent token ID", requiresTokenId: true, friction: "low", parameters: { type: "object", properties: { tokenId: { type: "string", description: "Agent token ID (numeric)" } }, required: ["tokenId"] } }),
  tool({ name: "agent_metadata", class: "read", label: "Agent Info", hint: "Get on-chain metadata for an agent (name, owner, data hash, description)", requiresTokenId: true, friction: "low", parameters: { type: "object", properties: { tokenId: { type: "string", description: "Agent token ID (numeric)" } }, required: ["tokenId"] } }),
  tool({ name: "event_history", class: "read", label: "Event History", hint: "Query recent on-chain events (Tick, Transfer, etc.)", friction: "low", parameters: { type: "object", properties: { eventName: { type: "string", description: "Filter by event name (Tick, Transfer)" }, limit: { type: "number", description: "Max events (default 20)" } } } }),
  ...
  tool({ name: "mint_agent", class: "encode", label: "Mint Agent", hint: "Mint a new agent NFT. Opens MetaMask for the transaction.", requiresWallet: true, encodeOnly: true, friction: "medium", parameters: { type: "object", properties: { dataDescription: { type: "string", description: "Human-readable agent name" }, dataHash: { type: "string", description: "Hex hash of the agent data" } }, required: ["dataDescription", "dataHash"] } }),
  tool({ name: "deposit", class: "encode", label: "Deposit", hint: "Deposit 0G into an agent vault. Opens MetaMask.", requiresWallet: true, requiresTokenId: true, encodeOnly: true, friction: "medium", parameters: { type: "object", properties: { tokenId: { type: "string", description: "Agent token ID" }, amount: { type: "string", description: "Amount in 0G (e.g. 1.5)" } }, required: ["tokenId", "amount"] } }),
  tool({ name: "withdraw", class: "encode", label: "Withdraw", hint: "Withdraw 0G from an agent vault. Opens MetaMask.", requiresWallet: true, requiresTokenId: true, encodeOnly: true, friction: "medium", parameters: { type: "object", properties: { tokenId: { type: "string", description: "Agent token ID" }, amount: { type: "string", description: "Amount in 0G (e.g. 0.5)" } }, required: ["tokenId", "amount"] } }),
