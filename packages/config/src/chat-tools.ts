...
export const CHAT_TOOL_CATALOG = [
  ...
  tool({ name: "mint_agent", class: "encode", label: "Mint Agent", hint: "Mint a new Axiom iNFT agent. Requires dataDescription (agent name). Opens MetaMask. dataHash optional — derived from name if omitted. Registers dataHash with the oracle when possible.", requiresWallet: true, friction: "medium", parameters: { type: "object", properties: { dataDescription: { type: "string", description: "Human-readable agent name (required)" }, dataHash: { type: "string", description: "Optional 0x-prefixed hash. Omitted → keccak of name." } }, required: ["dataDescription"] } }),
